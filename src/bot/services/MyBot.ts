...
import { ChatVideoData, SimplifiedFormat } from '../../shared/types/types';
import { videoInfo } from '@distube/ytdl-core';
import path from 'node:path';
import { deleteFile } from '../../shared/utils/utils';
...
export class MyBot extends TelegramBot {
  ...
  public async sendFormats(chatId: ChatId, link: string): Promise<number> {
    const BUTTONS_PER_ROW = 2;

    const response = await fetch(`http://localhost:3000/api/info?link=${link}`);

    const video = await response.json();

    const formats: SimplifiedFormat[] = video.simplifiedFormats;
    ...
    if (!thumbnailUrl) {
      return (await this.sendMessage(chatId, video.title, options)).message_id;
    }
